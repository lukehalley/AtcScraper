...
def collectPairs():
    ...
    collectedNetworkDexs = invokePoolWithTimeout(
        functionToRun=gatherNetworkDexs,
        functionArgs=networksToCollect
    )

    if collectedNetworkDexs:
        ...
        if collectedNetworks > 0 and collectedDexs > 0:
            ...
            collectedDexPairs = invokePoolWithTimeout(
                functionToRun=gatherPairsForDex,
                functionArgs=combinedDexs
            )

            if collectedDexPairs:
                ...
                if combinedDexPairs:
                    ...
                    # Collect all dex pairs
                    allPairsResults = invokePoolWithTimeout(
                        functionToRun=gatherTransactionsForPair,
                        functionArgs=combinedDexPairs
                    )
                ...
                if filteredTokensToGetDecimalsFor:
                    ...
                    decimalsRetrieved = invokePoolWithTimeout(
                        functionToRun=fillTokenDecimals,
                        functionArgs=filteredTokensToGetDecimalsFor
                    )

                    if decimalsRetrieved:
                ...
                if filteredPairsToGetAddressesFor:
                    ...
                    updatedResults = invokePoolWithTimeout(
                        functionToRun=fillPairAddresses,
                        functionArgs=filteredPairsToGetAddressesFor
                    )

                    if updatedResults:

                        updatedTokens = [updateResult for updateResult in updatedResults if updateResult]
