...
def decodeTx(transactionDetails):
    ...
    # Parent Dicts
    pairDetails = transactionDetails["pairDetails"]
    networkDetails = pairDetails["network"]
    dexDetails = pairDetails["dex"]
    ...
    # Network
    networkDbId = networkDetails["db"]["dbId"]
    networkRPCUrl = networkDetails["rpcUrl"]

    # Dex
    dexDbId = dexDetails["db"]["dbId"]
    dexRouterAddress = dexDetails["dex"]["abi"]["router"]
    dexRouterAbi = loadLocalABI(
        path=dexDetails["dex"]["abi"]["router_s3_path"]
    )

    # Transactions
    transactionHash = transactionDetails["txnHash"]

    web3 = Web3(Web3.HTTPProvider(networkRPCUrl))
    ...
    try:
        ...
        if transaction["to"] == dexRouterAddress:
            ...
            (contract, routerAbi) = getContract(dexRouterAddress, json.dumps(dexRouterAbi))
        ...
        printLog(
            msg=f"Unable To Decode Route For {pairName} ⛔️"
        )
        pass
